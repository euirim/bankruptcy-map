...
class Document:
    ...
    def get_thumbnail(self): #self should be a document
           url = self.get_file_url()
           if not self.is_available() and not url:
               return
           directory = 'tmp/{}'.format(self.get_id())
           ...
           first_pg_pdf = '{0}pg1.pdf'.format(directory)
           with open(first_pg_pdf, "wb") as out:
           ...
           fn = '_thumbnail.jpg'
           fn = '{0}{1}'.format(directory, fn)
           page_images[0].save(fn)
           
           #clean up
           os.remove(pdf_filename)
           os.remove(first_pg_pdf)
